...
KBrowser 在 `refid` 和 `selector` (Locator) 上均提供了“物理坐标模式”与“DOM JS 模式”：
...
2. **当遇到以下情况时，切换到 JS 模式 (`js*` 系列函数)**：
   * 目标元素被遮挡（例如被隐私策略弹窗、新用户红包或 cookie 确认框覆盖）。
...
尽管两者都支持物理和 JS 操作，但在基础定位逻辑上仍有根本区别：

* **selector (Locator)**：每次操作时重新计算并动态查找元素，在复杂的动态网页或 DOM 树频繁变动时最为安全。
* **refid**：直接绑定至某一特定快照，利用缓存好的节点定位元素。在短时间内的密集流程化操作中，能够提供极高的响应速度。

关于两者的操作方法和适用模式，参见上文的对比表格和选择指南。
